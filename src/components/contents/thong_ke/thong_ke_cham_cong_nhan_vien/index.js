...
import {
  useDownloadExcelChamCongTheoThangMutation,
  useGetChamCongMoiThangQuery,
  useLazyDownloadExcelChamCongTheoThangQuery,
} from "../../../../services/chamCongApis";
...
const ThongKeChamCongNhanVienContent = () => {
  ...
  const [
    triggerDownload,
    {
      isLoading: downloadExcelThongKeChamCongIsLoading,
      isFetching: downloadExcelThongKeChamCongIsFetching,
    },
  ] = useDownloadExcelChamCongTheoThangMutation();
  const { data: allTenPhongBan } = useGetAllTenPhongBanQuery();

  const {
    data: allNhanVienChamCong,
    error: allNhanVienChamCongError,
    isLoading: allNhanVienChamCongIsLoading,
    isFetching: allNhanVienChamCongIsFetching,
  } = useGetChamCongMoiThangQuery([selectedYear, selectedMonth]);
  ...
  const handleClickChiTietChamCongNhanVien = (id, ten_nhan_su, ma_nhan_su) => {
    navigate(`/thong_ke/cham_cong_nhan_vien/${id}`, {
      state: {
        year: selectedYear,
        month: selectedMonth,
  ...
  const handleClickDownloadExcelChamCong = async () => {
    await triggerDownload({ chamCongs: searchData })
      .unwrap()
      .then((response) => {
  ...
  useEffect(() => {
    if (
      downloadExcelThongKeChamCongIsLoading ||
      downloadExcelThongKeChamCongIsFetching
    ) {
      setIsLoading(true);
    } else {
      setIsLoading(false);
  ...
  const columns = [
...

    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <EyeOutlined
            style={{ fontSize: "18px", margin: "0 10px", cursor: "pointer" }}
            onClick={() =>
              handleClickChiTietChamCongNhanVien(
                record.nhan_vien_id,
                record.ten_nhan_su,
                record.ma_nhan_su
              )
        ...
        <h1>
          ...
          <div style={{ float: "right" }}>
            <Button
              ...
              onClick={handleClickDownloadExcelChamCong}
            >
              Download excel
            </Button>
          </div>
